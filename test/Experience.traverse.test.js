...
describe('traverse()', () => {
	...
	test('Nulling any argument becomes the default value', () => {
		const experience1 = new Experience(basicExperience);
		experience1.register('self::div', ({ key, traverse, proof }) => (
			<x-ok key={ key() } data-proof={ proof }>{ traverse() }</x-ok>
		));
		expect(renderer.create(experience1.render(sync(xml), { proof: 'blaat'})).toJSON())
			.toMatchSnapshot();

		const experience2 = new Experience(basicExperience);
		experience2.register('self::div', ({ key, traverse, proof }) => (
			<x-ok key={ key() } data-proof={ proof }>{ traverse(null, null) }</x-ok>
		));
		...
		experience3.register('self::div', ({ key, traverse, proof }) => (
			<x-ok key={ key() } data-proof={ proof }>{ traverse('./node()', { proof }) }</x-ok>
		));
	...
	test('Just the first argument is also optional', () => {
		const experience1 = new Experience(basicExperience);
		experience1.register('self::div', ({ key, traverse, proof }) => (
			<x-ok key={ key() } data-proof={ proof }>{ traverse(null, { proof }) }</x-ok>
		));
		expect(renderer.create(experience1.render(sync(xml), { proof: 'blaat'})).toJSON())
			.toMatchSnapshot();
		...
		const experience2 = new Experience(basicExperience);
		experience2.register('self::div', ({ key, traverse, proof }) => (
			<x-ok key={ key() } data-proof={ proof }>{ traverse({ proof }) }</x-ok>
		));
